...
func _physics_process(delta):
	...
	# Guard: wait until Vehicle's @onready vars are initialized
	if target == null or target.vehicle_model == null:
		return
	...
	# ── 4. Speed-based zoom on child Camera node ────────────────────────────
	var speed_factor: float = clamp(abs(target.linear_speed), 0.0, 1.0)
	var target_z: float     = remap(speed_factor, 0.0, 1.0, 0.0, -2.0)
